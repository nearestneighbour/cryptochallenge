...
def get_sk_from_sig(dd):
    pass

class dsa:
    def __init__(self, p=p, q=q, g=g, hashfunc=intdigest):
        self.p = p
        self.q = q
        self.g = g
    ...
    def sign(self, msg, x=None):
        ...
        s = invmod(k * (self.hashfunc(msg) + x*r) % self.q, self.q)
        assert s != None, "sdfsdffdsf"
        return r, s

    def verify(self, msg, r, s, y=None):
        y = y if y else self.y
        assert r in range(1, self.q) and s in range(1, self.q), "Invalid signature"
        w = invmod(s, q)
        u1 = self.hashfunc(msg) * w % self.q
        u2 = r * w % self.q
...
def main():
    ...
    r = int('548099063082341131477253921760299949438196259240', 16)
    s = int('857042759984254168557880549501802188789837994940', 16)
